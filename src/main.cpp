...
#if defined(NDEBUG)
#error "GyMoney cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight) {
    ...
    if (nHeight == 0) {
        nSubsidy = 5 * COIN;
    } else if (nHeight <= 4 && nHeight >= 1) {
        nSubsidy = 5000000 * COIN;
    } else if (nHeight <= 25004 && nHeight >= 5) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <= 53576 && nHeight >= 25005) {
        nSubsidy = 3.5 * COIN;
    } else if (nHeight <= 86910 && nHeight >= 53577) {
        nSubsidy = 3 * COIN;
    } else if (nHeight <= 126911 && nHeight >= 86911) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight <= 176912 && nHeight >= 126912) {
        nSubsidy = 2 * COIN;
    } else if (nHeight >= 176913) {
        nSubsidy = 1.5 * COIN;
    }
    return nSubsidy;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck) {
    ...
    if (pindex->nHeight > Params().LAST_POW_BLOCK() && block.IsProofOfWork())
        return state.DoS(100, error("ConnectBlock() : PoW period ended"),
            REJECT_INVALID, "PoW-ended"); //gmychange
