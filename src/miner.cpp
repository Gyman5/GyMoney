...
#include "init.h" // gmychange
#include "script/sign.h" // gmychange
...
//                return NULL;
        ...
        //pblocktemplate->vTxFees[0] = -nFees; // gmychange

        // Fill in header
        pblock->hashPrevBlock = pindexPrev->GetBlockHash();
        if (!fProofOfStake) // gmychange
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GyMoneyMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GyMoneyMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake) {
    LogPrintf("GyMoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GyMoney-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running GyMoneyMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
