...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::GMY)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("GMY")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeGyMoneyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGyMoneyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeGyMoneyAmount = strAnonymizeGyMoneyAmount.remove(strAnonymizeGyMoneyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGyMoneyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeGyMoneyAmount * COIN) nMaxToAnonymize = nAnonymizeGyMoneyAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeGyMoneyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeGyMoneyAmount));
        strAnonymizeGyMoneyAmount = strAnonymizeGyMoneyAmount.remove(strAnonymizeGyMoneyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGyMoneyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
